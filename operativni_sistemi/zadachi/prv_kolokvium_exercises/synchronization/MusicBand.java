...
public class MusicBand {
	...
	static Semaphore sHere = null;
	static Semaphore gHere = null;
	static Lock lockS = null;
	static Lock lockG = null;
	static int countG;
	static int countS;
	static Semaphore waitToLeave = null;
	static Semaphore gReady = null;
	static Semaphore sReady = null;

	public static void init() {
		// TODO: da se implementira
		sHere = new Semaphore(2);
		gHere = new Semaphore(3);
		sReady = new Semaphore(0);
		gReady = new Semaphore(0);
		lockS = new ReentrantLock();
		lockG = new ReentrantLock();
		countG = 0;
		countS = 0;
		waitToLeave = new Semaphore(0);
	}
...
class MusicBandState extends AbstractState {
	...
	public MusicBandState() {
		guitarPlayer = new BoundCounterWithRaceConditionCheck(0, 3, MAXIMUM_3_GUITAR_PLAYERS_POINTS,
				MAXIMUM_3_GUITAR_PLAYERS, null, 0, null);
		singer = new BoundCounterWithRaceConditionCheck(0, 2, MAXIMUM_2_SINGER_POINTS, MAXIMUM_2_SINGER, null, 0, null);
	}
...
//	      log(new PointsException(5,"Ne se prisutni site"),"");
		...
		synchronized (this) {
			if (groupMembersCount == 5) {
				if (guitarPlayer.getValue() == 3&&singer.getValue() == 2) {
					...
					log(new PointsException(GROUP_NOT_FORMED_POINTS, GROUP_NOT_FORMED), null);
	...
	public synchronized void finalize() {
		if (guitarPlayer.getMax() == 1 && singer.getMax() == 1) {
			logException(new PointsException(PLAYING_NOT_PARALLEL_POINTS, PLAYING_NOT_PARALLEL));
		}
	}
...
abstract class AbstractState {
	...
	/**
	 * Method called after threads ended their execution to validate the correctness
	 * of the scenario
	 */
	...
	/**
	 * 
	 * @return if the current thread is instance of TemplateThread it is returned,
...
class BoundCounterWithRaceConditionCheck {
	 ...
	 *                        violation
	 * @param maxErrorMessage message shown when the upper bound constrain is
	 *                        violated
	 * @param minAllowed      lower bound of the value
	 * @param minErrorPoints  how many points are lost with the min value constraint
	 *                        violation
	 * @param minErrorMessage message shown when the lower bound constrain is
	 *                        violated
	 */
	public BoundCounterWithRaceConditionCheck(int value, Integer maxAllowed, int maxErrorPoints, String maxErrorMessage,
	...
	/**
	 * 
	 * @return the maximum value of the integer variable that occurred at some point
	 *         of the execution
	 */
	public int getMax() {
		return max;
	...
	/**
	 * Testing for race condition. NOTE: there are no guarantees that the race
	 * condition will be detected, but higher the time argument is, the probability
	 * for race condition occurrence is higher
	 * 
	 * @return
	 */
	public PointsException checkRaceCondition(int time, String message) {
		...
		if (val != value) {
			PointsException e = new PointsException(RACE_CONDITION_POINTS, message);
			return e;
		}
		return null;
...
class Switcher {
	...
	/*
	 * This method pauses the current thread i.e. changes its state to be Blocked.
	 * This should force thread switch if there are threads waiting
	 */
